...
### Self-Simulating Reality Agent (1.18.4 · juli 2026 — V0+V1+V2+V3)
...
Plassert i sidemenyen under **Future Item Agents** (ikon 🌀). **10 faner**:
...
5. 🔍 **WiPhy Search** *(NY · V1)* — live spørringer mot `wiphy.org/api/search`, Pasterskis offentlige MCP-server for gjenfinning av fysikk-påstander (~10 155 artikler · 361 273 påstander · 17 953 konsepter). Korpus-statistikkblokk ved montering + defensiv JSON-parser (godtar flere feltnavn- og wrapper-former) + differensiert `cors` / `http` / `network`-feiltekst med "Open on wiphy.org"-utvei. Artikkel-ID-er lenker til arXiv.
6. 🔬 **Claim Analyzer** *(NY · V2)* — lim inn en sterk påstand (f.eks. *"bevissthet kollapser bølgefunksjonen"*) og backend-LLM-en returnerer 5 paneler: **epistemisk dom**-badge (`mostly_solid / mixed / mostly_overreach / unsupported`), **vitenskapelig kjerne** (hver del med sitt bevisnivå), **overreach** (hver del merket med én av 5 typer: `unsupported / category_error / conflation / overgeneralization / philosophical_leap`), **reformulering** i samme idés ærlige register, og **nøkkelbegreper** som klikkbare chips. Klikk på et begrep bygger bro til WiPhy Search-fanen med søket auto-kjørt via en shell-nivå `{query, nonce}`-tilstand.
7. 🎨 **Playground** *(NY · V3)* — to matchede pedagogiske verktøy stablet vertikalt: **Theory Map** (ren SVG, 8 noder med OPH sentrert og 7 satellitter håndplassert etter strukturell affinitet; 9 typede kanter med distinkte farger: `provides_form`, `structural_parallel`, `competes_with`, `candidate_measure`, `de_mystifies`, `different_framing`, `supports_side`, `extends_to_flat`; klikk på en node → info-panel som leser fra samme i18n-nøkler som Theory Tour) og **Observer Patch Simulator** (HTML5 Canvas 720×380 med `requestAnimationFrame` + `cancelAnimationFrame`-opprydding; N patches med brownsk bevegelse + veggsprett + parvis-overlapp tilstandskonvergens; live konsensus-metrikk `1 - std(states)` vokser mot 100% uten noen global koordinator — den pedagogiske poengen).
8. 🧠 **AI as Observer** — 5 tankeeksperimenter om hvorvidt AI-er kan være observer patches
9. 🌌 **Substrate Question** — 7 filosofiske seksjoner som utvider OPHs ontologi (det harde problemet, substrat vs opplevelse, kosmologisk konvergens, hypotesen om rekursiv forståelse, det platonske spørsmålet, den lingvistiske grensen, tre ærlige posisjoner)
10. 🗺️ **Roadmap & Sources** — V0→V3-faser + 14 referanselenker (OPH repo/learn/book, Mueller X, Bostrom, Rovelli, Susskind, Friston, IIT, GNW, **Pasterski Perimeter, Simons Collaboration, arXiv 2111.11392, physicsgirl.com**) + blokk med kandidat-integrasjoner for full `wiphy.org/mcp` MCP-verktøyintegrasjon (avventer backend-arbeid)
...
- **Fane 1 — AGI Progress Tracker**: CHC-inspirert rammeverk fra Hendrycks et al. (2025). Datasettet utvidet fra 2 til **5 modeller**: GPT-4 (2023) 27%, GPT-5 (2025) 58%, **Claude Opus 4.6 (2025) 61%**, **Gemini 3.1 Pro (2026) 61%**, **Claude Opus 4.7 (2026) 67%**. Hver modell bærer ekte benchmark-notater (GPQA Diamond, MATH-500, SWE-bench Verified/Pro, ARC-AGI-2). Long-Term Memory Storage forblir 0 på tvers av alle nåværende LLM-er — den arkitektoniske flaskehalsen fra artikkelen.
