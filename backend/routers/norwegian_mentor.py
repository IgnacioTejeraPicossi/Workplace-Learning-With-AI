"""
Norsk Mester AI — Router
========================
REST API for the Norwegian mentor (Language Agents group).
"""
...
try:
    from backend.services.norwegian_mentor import (
        get_pronunciation, get_grammar_path, get_grammar_point, get_smaord,
        get_vocab_all, srs_due, srs_review, get_bridge, get_culture,
        get_overview, conversation_message, scenarios_catalogue,
        writing_feedback, health,
    )
except ImportError:  # pragma: no cover
    from services.norwegian_mentor import (  # type: ignore
        get_pronunciation, get_grammar_path, get_grammar_point, get_smaord,
        get_vocab_all, srs_due, srs_review, get_bridge, get_culture,
        get_overview, conversation_message, scenarios_catalogue,
        writing_feedback, health,
...
router = APIRouter(prefix="/api/norwegian")


class SrsReviewRequest(BaseModel):
    vocab_id: str = Field(..., description="Vocabulary item id (e.g. 'nvocab_001')")
    grade: str = Field(..., pattern=r"^(again|good|easy)$")


class ConversationMessageRequest(BaseModel):
    scenario: str = Field("smalltalk")
    difficulty: str = Field("b2", pattern=r"^(b1|b2|c1)$")
...
@router.get("/pronunciation", summary="Sounds + tonelag (pitch accent)")
...
@router.get("/smaord", summary="The tiny filler words (jo, da, vel, altså…)")
def smaord_endpoint() -> Dict[str, Any]:
    items = get_smaord()
    return {"count": len(items), "items": items}


@router.get("/vocab/all", summary="Full Bokmål vocabulary list")
...
@router.get("/bridge", summary="Germanic Bridge — EN↔NO cognates + false friends")
def bridge_endpoint() -> Dict[str, Any]:
    return get_bridge()


@router.get("/culture", summary="Culture notes (Janteloven, dugnad, kos…)")
...
@router.post("/conversation/message", summary="Send a turn to the Norwegian mentor")
...
@router.post("/writing/feedback", summary="Get B2/C1 feedback on a Norwegian text")
