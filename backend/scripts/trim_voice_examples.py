...
def regen_and_retrim(ids: List[str], profile: str) -> List[str]:
    """Regenerate the given ids from scratch, then trim again. Returns ids that
    still fail after regeneration."""
    import httpx
    ...
    with httpx.Client() as client:
        ...
        for eid in ids:
            ...
            print(f"[regen] {eid}: {text[:40]!r} ...", flush=True)
            try:
                ...
                continue
            # Overwrite the pristine backup with the fresh generation, then trim.
            os.makedirs(_RAW_DIR, exist_ok=True)
            with open(os.path.join(_RAW_DIR, f"{eid}.wav"), "wb") as f:
                f.write(wav_bytes)
            out, info = trim_leading_prefix(wav_bytes, expected_chars=len(text))
            if not info["ok"]:
                print(f"    [still bad] {eid}: {info['reason']}", flush=True)
                # Keep the untrimmed fresh generation so at least it plays.
                with open(example_wav_path(eid), "wb") as f:
                    f.write(wav_bytes)
                items[eid]["duration"] = round(info["orig_sec"] or 0, 2)
                items[eid]["trimmed"] = False
                still_failing.append(eid)
            else:
                with open(example_wav_path(eid), "wb") as f:
                    f.write(out)
                ...
                print(f"    [OK] {eid}: → {info['new_sec']}s", flush=True)
...
def main() -> int:
    ...
    if failures and args.regen_failures:
        print(f"\n[info] regenerating {len(failures)} failed file(s): {failures}")
        failures = regen_and_retrim(failures, args.profile)
