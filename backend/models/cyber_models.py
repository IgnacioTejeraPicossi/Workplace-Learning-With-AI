...
class CyberRAGResponse(BaseModel):
    ...
    # True when the answer came from the canned fallback (no LLM available),
    # False when generated by the real model grounded in module content.
    is_mock: bool = False
...
class ComplianceUpdateRequest(BaseModel):
    # Validated: an arbitrary status string would silently corrupt the
    # compliance summary counters (422 instead).
    status: Literal["implemented", "partial", "not_implemented", "not_applicable"]
