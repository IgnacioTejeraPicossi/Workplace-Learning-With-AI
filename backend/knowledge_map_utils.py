...
async def extract_topics_from_modules():
    """Extract unique topics from all modules using direct MongoDB access (like Dashboard)"""
    all_topics = {}
    
    print("🔍 Starting topic extraction from MongoDB collections (like Dashboard)...")
    
    # FORCE direct MongoDB access like Dashboard does for complete data
    print("📚 Fetching micro-lessons directly from MongoDB...")
    ...
    print("🎥 Fetching videos directly from MongoDB...")
    ...
    for video in videos:
        if video.get("topic"):
            ...
            if topic_key not in all_topics:
                ...
                print(f"  + Added video topic: {video['topic']} (key: {topic_key})")
            else:
                all_topics[topic_key]["count"] += 1
