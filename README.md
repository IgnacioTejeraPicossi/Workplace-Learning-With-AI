...
| **Specialized AI Use Cases testing** | J-messages Analyzer, compliance/productivity agents, ATM V&V Test Copilot, AI experimentation |
...
| # | Tab | Purpose |
|---|-----|---------|
| 1 | 📊 Dashboard | Live quality status: total runs, pass rate, open findings, critical blockers, 11 quality gates, 16-button Quick Actions panel, **Phase F: Tom's tooling tips banner** (NextJS + Storybook + Playwright + Postman for the rodekors.no rebuild) |
...
| 3 | 🎭 Playwright | Cross-browser E2E generator (10 scopes: navigation, forms, search, donation, volunteer, CMS preview, a11y smoke, visual, API mock, **Phase F: Storybook for Designsystemet components — generates a deterministic `storybook.spec.ts` with `@storybook/test-runner` patterns, `iframe.html` URL pattern, axe-core injection per story, WCAG 2.2 AA tag profile**). Tom-tip banner explains Playwright is bundled with Storybook for this project. |
| 4 | 🌲 Cypress | Component + frontend regression generator. **Phase F: soft deprecation notice** at the top recommending Playwright (Tab 3) since Storybook bundling is already in place; Cypress remains for ad-hoc / non-Storybook needs. |
| 5 | 🔌 API QA | Enonic Guillotine GraphQL + NextJS API + external integrations — 10 checks. **Phase F: GraphQL schema introspection panel** (lists 5 Guillotine operations + 8 Røde Kors content types like Distrikt / Aktivitet / Kampanje, mock-first) + **Postman Collection v2.1 export button** with browser download (4 canonical queries, `base_url` + `token` variables, per-request tests asserting status 200 + no GraphQL errors). Tom's preferred workflow for poking at the XP backend. |
...
| 9 | ♿ Universell utforming-pilot (Accessibility) | **Tool selector** with 3 engines side by side: 🤖 **axe-core + Lighthouse** (existing automated scan, 12 checks, WCAG 2.1/2.2 AA toggle), 🔊 **NVDA** (Phase G — generates a markdown checklist with `Insert+T` / `Insert+F7` / Tab keystrokes + expected announcements + WCAG SC mapping per step, 5 scope presets: donation / volunteer / search / navigation / forms, .md download), 🌊 **WAVE (WebAIM)** (Phase G — mock-first report with errors/contrast/alerts/features/structure/aria stat cards + 3 detail tables + deep link to `wave.webaim.org/report#/{url}`; real API call when `WAVE_API_KEY` env var is set) |
...
| 14 | 🛡️ Sikkerhet og personvern (Security & Privacy) | **Phase H · Pack 2 — full backend-driven workbench**: snapshot panel (overall status + pass/warn/fail/open findings) → clickable check cards (status + severity + scan_type + category badge + linked findings count) → detail drawer (summary, evidence, recommendations, full findings rows) → filterable findings list with **4-state workflow** (open / accepted_risk / fixed / verified, status preserved across re-scans) and owner/recommendation editing + audit history → **scan history** with trend arrows comparing fail+warn vs previous run → structured **DPIA editor** with 10 fields (purpose, dataTypes, sensitiveData, storageLocation, accessRoles, retention, thirdParties, legalBasis, riskNotes, mitigations). 13 security/privacy + 12 DPIA checks = 25 total. Backed by 3 MongoDB collections + in-memory fallback |
...
- Router: `backend/routers/red_cross_qa.py` — **37 routes** at `/api/red-cross-qa/*` (Phase B: DPIA / DoD / Resilience / UAT / Risk Matrix; Phase D: Loadster; Phase F: Postman + GraphQL introspection; Phase G: `/generate-nvda-script`, `/run-wave-audit`)
- **Phase H sub-router** `backend/routers/qa_security.py` — **13 paths / 15 method bindings** at `/api/qa/security/*`. Pack 2: status, checks, checks/{id}, scan, findings, findings/{id} PATCH, history, dpia GET/POST/PATCH. Pack 3: `/export/markdown`, `/findings/{id}/dispatch-ado`, `/diff`, `/findings/{id}/verify`, `/environments`. Backed by `backend/services/qa_security_service.py` + `backend/repositories/qa_security_repository.py` + `backend/schemas/qa_security.py` with 3 dedicated MongoDB collections (`qa_security_scans`, `qa_security_findings`, `qa_security_dpia`) and in-memory fallback. User-set finding statuses (`fixed` / `accepted_risk` / `verified`) are preserved across re-scans. Pack 3 adds: Markdown sprint report export, idempotent single-finding ADO dispatch (deterministic mock work-item IDs), scan-run diff with new/fixed/regressed/persisted buckets, verify-fix re-scan flow with auto-status-transition, and 4-environment governance matrix. **Pack 4.1** adds per-scan `findings_snapshot` on every `ScanRun` doc → `diff_scans` returns a `diff_mode` discriminator (`"precise"` when both runs carry snapshots, `"timestamp_fallback"` otherwise). **Pack 4.2** wires real Azure DevOps REST integration: when `ADO_PAT` (or `AZURE_DEVOPS_PAT`) is set in env, `dispatch_finding_to_ado` POSTs a JSON-Patch document to `https://dev.azure.com/{org}/{project}/_apis/wit/workitems/${type}?api-version=7.0` (Basic auth, empty user + PAT as password); on any failure it gracefully falls back to the deterministic SHA mock and surfaces `is_mock` + `live_error` on the response. UI renders a MOCK / LIVE badge next to the ADO link accordingly.
...
**i18n**: Full EN / NO / ES parity (40+ top-level sections each, **700 keys per locale** after Phase H Pack 4.2). New blocks: `dpia:` (10 keys), `dod:` (15), `resilience:` (13), `uatSupport:` (22), `riskMatrix:` (24), `stakeholders:` (3), Phase C migration provenance + WCAG version (8), Phase D Loadster tool selector (11) + 2 tab labels, Phase F: Tom-tip banners + Storybook + Postman + GraphQL introspection (27), Phase G NVDA + WAVE (29), **Phase H · Pack 2: snapshot panel labels + clickable check cards + 4-state finding workflow (open / accepted_risk / fixed / verified) + 5 filters (status / type / category / finding state / severity) + scan history with trend arrows + structured DPIA editor with 10 fields (82 keys)**.
...
**Validation status**: Backend imports clean (23 suites, 37 routes + 13 Phase H paths / 15 method bindings after Pack 3), end-to-end smoke tests pass (`backend/tests/smoke_red_cross_qa.py` — **37 checks** covering Phases A→G + Phase H+ skill integration across 13 audit areas; **`backend/tests/smoke_qa_security.py` — 16 checks** covering Phase H Pack 2 + Pack 3 + Pack 4 lifecycle: perform_scan persistence, deterministic finding IDs, filterable findings, **re-scan preserves user-set status** (Pack 2 promise), **Markdown export with required sections**, **idempotent ADO dispatch with deterministic mock work-item ID** + Pack 4.2 `is_mock` / `live_error` shape + JSON-Patch document validation, **scan-diff with 4 buckets** + Pack 4.1 `diff_mode == "precise"` snapshot-driven assertion, **verify-fix flow with status transitions**, **environment matrix with worst-overall aggregation**, DPIA seed → save → patch). All 3 locales parse with parity (**721 keys × 3** after Phase H+ Enonic-skill integration cycle), frontend production build succeeds with **0 warnings in `src/red-cross-qa/`**.

**Phase H+ (1.15.0, 2026-05-21) — Enonic XP skill systematically applied across 13 audit areas**. The `.claude/skills/enonic-xp/` knowledge base (security / performance / reliability / api-design / data-integrity / nashorn-compatibility patterns) drove additive enrichments in every suite of the module: 34 new skill-aligned checks, 6 new deterministic Playwright/Cypress specs, 5 in-memory baseline trackers for trend detection (`_GRAPHQL_BASELINES`, `_PERF_HOT_QUERY_BASELINES`, `_DS_COMPLIANCE_BASELINES`, `_ROLE_MATRIX_BASELINES`, `_RESILIENCE_BASELINES`), `cross_tool_refs` on 9 areas making each response self-navigable, and 2 Optional fields on the Phase H `Finding` schema (`enonic_xp_pattern` + `automation_ref`) so every finding can cite its skill section and link to an existing automated spec. Full retrospective in `docs/audits/red-cross-qa-enonic-xp-roundup.md`.

**AGI Progress · Feedback trilogy (1.15.1 + 1.15.2, 2026-05-22) — A + B + C complete & interconnected**. The Homo Sapiens vs. KI i Test workshop tab originally shipped Option B (ephemeral re-run, Pack 3) and Option C (persistent prompt evolution + governance + regression harness, Phase E). The remaining gap — **Option A (log-only feedback persistence)** — landed in 1.15.1 with a new `homo_vs_ai_feedback_log` Mongo collection (in-memory fallback for workshop demos), auto-logging from Re-run with feedback + Propose revision actions, and a one-click JSON export panel at the bottom of the workshop tab. 1.15.2 then closed the **A → C bridge**: every captured note can be promoted to a Phase E revision proposal via a per-row `🧬 Promote to revision` button in the export panel, calling the existing `proposePromptRevision` endpoint with the entry's stored fields (`task`, `user_input`, `previous_ai_output`, `text`) — no re-typing required. Smoke `smoke_feedback_log.py` 11/11 PASS. Phase E unchanged (3/3). Full retrospective in `docs/audits/agi-progress-trilogy-roundup.md`.
...
5. 💬 **Dialogue** *(NEW · V1 [1.36.0])* — the conversational agent. Ask about observers, minds, consciousness, simulation, holography or OPH; **every answer comes back structured and tagged by evidence level** — never bare prose that could pass speculation off as fact. `POST /api/self-sim-reality/chat` grounds each turn in a **curated OPH + science knowledge base** (12 chunks, each with its own epistemic level + citations) retrieved by keyword overlap ("RAG-lite" — a vector store is the documented upgrade seam). Structured JSON: `short_answer` + level-tagged `sections` (scientific_grounding / speculative_extension / oph_interpretation) + red-team `objections` + `safer_reformulation` (when the question over-claims) + `suggested_next_question`. Off-palette levels are clamped; trilingual mock keeps the tab alive offline. The system prompt enforces the module rule (*never "this is true", always "this belongs to level X"*; OPH always `speculative`) and flags the observer-as-device vs observer-as-mind conflation.
6. 🔍 **WiPhy Search** — live queries against `wiphy.org/api/search`, Pasterski's public MCP server for physics-claim retrieval (~10 155 papers · 361 273 claims · 17 953 concepts). Corpus stats block on mount + defensive JSON parser (accepts multiple field-name and wrapper shapes) + differentiated `cors` / `http` / `network` error copy with "Open on wiphy.org" escape hatch. Paper IDs linkify to arXiv.
7. 🔬 **Claim Analyzer** *(V2)* — paste a strong claim (e.g. *"consciousness collapses the wavefunction"*) and the backend LLM returns 5 panels: **epistemic verdict** badge (`mostly_solid / mixed / mostly_overreach / unsupported`), **scientific core** (each part with its evidence level), **overreach** (each part labelled with one of 5 types: `unsupported / category_error / conflation / overgeneralization / philosophical_leap`), **reformulation** in the same idea's honest register, and **key terms** as clickable chips. Clicking a term bridges to the WiPhy Search tab with the search auto-run via a `{query, nonce}` shell-level state.
8. 🎨 **Playground** *(V3)* — two matched educational tools stacked vertically: **Theory Map** (pure SVG, 8 nodes with OPH centered and 7 satellites hand-positioned by structural affinity; 9 typed edges with distinct colours: `provides_form`, `structural_parallel`, `competes_with`, `candidate_measure`, `de_mystifies`, `different_framing`, `supports_side`, `extends_to_flat`; click a node → info panel reading from the same i18n keys as Theory Tour) and **Observer Patch Simulator** (HTML5 Canvas 720×380 with `requestAnimationFrame` + `cancelAnimationFrame` cleanup; N patches with brownian motion + wall bounces + pairwise-overlap state convergence; live consensus metric `1 - std(states)` grows toward 100% without any global coordinator — the pedagogical punchline).
9. 🧠 **AI as Observer** — 5 thought experiments about whether AIs can be observer patches
10. 🌌 **Substrate Question** — 7 philosophical sections extending OPH's ontology (hard problem, substrate vs experience, cosmological convergence, recursive-understanding hypothesis, Platonic question, linguistic boundary, three honest positions)
11. 🗺️ **Roadmap & Sources** — V0→V3 phases + 14 reference links (OPH repo/learn/book, Mueller X, Bostrom, Rovelli, Susskind, Friston, IIT, GNW, **Pasterski Perimeter, Simons Collaboration, arXiv 2111.11392, physicsgirl.com**) + candidate integrations block for full `wiphy.org/mcp` MCP tool integration (pending backend work)

**Backend**:
- `backend/services/self_sim_reality_chat.py` + `backend/routers/self_sim_reality.py` *(V1 [1.36.0])* — `POST /api/self-sim-reality/chat` (+ `GET /concepts`, `GET /health`). Curated 12-chunk knowledge base retrieved by keyword overlap (RAG-lite), structured epistemically-tagged JSON with sanitize/clamp, trilingual mock. Tests `backend/tests/test_self_sim_reality_chat.py` (6 offline).
...
- **Tab 1 — AGI Progress Tracker**: CHC-inspired framework from Hendrycks et al. (2025). Dataset now spans **14 models, refreshed through August 2026** (web-sourced where past the assistant's knowledge cutoff): GPT-4 (2023) 27% → GPT-5 (2025) 58% → **Claude Opus 4.6/4.7/4.8** (2025–26) 61/67/71% → **Gemini 3.1 Pro** 61% → the Chinese open-weight **Kimi K2.6/K3** (Moonshot) 59/66% → **GPT-5.6 "Sol"** 75% → **Claude Opus 5** 76% → **Gemini 3.6 Flash** 66% → **Grok 4.5/4.6** (xAI/SpaceXAI) 68/73% → **Claude Fable 5** 80% (gated behind usage credits). Each model carries real benchmark notes (GPQA Diamond, SWE-bench Verified/Pro, ARC-AGI-2, Artificial Analysis Intelligence Index). Long-Term Memory Storage remains 0 across all current LLMs — the architectural bottleneck from the paper. Honest caveat surfaced in the notes: text-first open-weight models (Kimi) can lead coding benchmarks yet score lower on *this* breadth metric because vision and audio weigh equally with the other domains.
