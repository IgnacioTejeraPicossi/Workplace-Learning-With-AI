...
| **QA & Test Automation** | Red Cross Web QA Agent (20 tabs / 45 endpoints): Playwright + Cypress generators, Skjemabygger + Fundy forms QA, content migration audit, Enonic-specific perf, Designsystemet (Digdir) compliance, role permission matrix, **Azure DevOps work-item bundle**, **Sprint Report generator**, **DPIA / DoD verifier / Resilience / UAT-støtte / Risk Matrix**, **k6 + Loadster dual load-testing**, **Storybook + Postman + GraphQL introspection** (Phase F), **NVDA + WAVE** accessibility (Phase G), **Phase H · Pack 2 — Sikkerhet og personvern workbench** (backend-driven: stable check + finding contracts, MongoDB persistence, clickable detail panel, status transitions open/accepted_risk/fixed/verified preserved across re-scans, scan history with trend arrows, structured DPIA form with 10 fields), aligned to Trine Bruu's Teststrategi 30.3 |
...
- **Phase H · Pack 2 sub-router** `backend/routers/qa_security.py` — **8 paths / 10 method bindings** at `/api/qa/security/*` (status, checks, checks/{id}, scan, findings, findings/{id} PATCH, history, dpia GET/POST/PATCH) backed by `backend/services/qa_security_service.py` + `backend/repositories/qa_security_repository.py` + `backend/schemas/qa_security.py` with 3 dedicated MongoDB collections (`qa_security_scans`, `qa_security_findings`, `qa_security_dpia`) and in-memory fallback. User-set finding statuses (`fixed` / `accepted_risk` / `verified`) are preserved across re-scans.
...
**Validation status**: Backend imports clean (23 suites, 37 routes + 8 Phase H paths), end-to-end smoke tests pass (`backend/tests/smoke_red_cross_qa.py` — **20 checks** covering Phases A→G; **`backend/tests/smoke_qa_security.py` — 10 checks** covering Phase H lifecycle: perform_scan persists ScanRun + Findings; check shape carries category + scan_type + severity; finding ids are deterministic; get_status snapshot consistent; get_check_detail attaches findings_detail; filterable findings list; PATCH transitions open → fixed; **re-scan preserves user-set status** (the Pack 2 promise); history is newest-first; DPIA seed → save → patch works; router has 8 paths with GET+POST+PATCH on /dpia). All 3 locales parse with parity (**672 keys × 3**), frontend production build succeeds with **0 warnings in `src/red-cross-qa/`**.
