...
- **Phase H sub-router** `backend/routers/qa_security.py` — **13 paths / 15 method bindings** at `/api/qa/security/*`. Pack 2: status, checks, checks/{id}, scan, findings, findings/{id} PATCH, history, dpia GET/POST/PATCH. Pack 3: `/export/markdown`, `/findings/{id}/dispatch-ado`, `/diff`, `/findings/{id}/verify`, `/environments`. Backed by `backend/services/qa_security_service.py` + `backend/repositories/qa_security_repository.py` + `backend/schemas/qa_security.py` with 3 dedicated MongoDB collections (`qa_security_scans`, `qa_security_findings`, `qa_security_dpia`) and in-memory fallback. User-set finding statuses (`fixed` / `accepted_risk` / `verified`) are preserved across re-scans. Pack 3 adds: Markdown sprint report export, idempotent single-finding ADO dispatch (deterministic mock work-item IDs), scan-run diff with new/fixed/regressed/persisted buckets, verify-fix re-scan flow with auto-status-transition, and 4-environment governance matrix.
...
**Validation status**: Backend imports clean (23 suites, 37 routes + 13 Phase H paths / 15 method bindings after Pack 3), end-to-end smoke tests pass (`backend/tests/smoke_red_cross_qa.py` — **20 checks** covering Phases A→G; **`backend/tests/smoke_qa_security.py` — 15 checks** covering Phase H Pack 2 + Pack 3 lifecycle: perform_scan persistence, deterministic finding IDs, filterable findings, **re-scan preserves user-set status** (Pack 2 promise), **Markdown export with required sections**, **idempotent ADO dispatch with deterministic mock work-item ID**, **scan-diff with 4 buckets**, **verify-fix flow with status transitions**, **environment matrix with worst-overall aggregation**, DPIA seed → save → patch). All 3 locales parse with parity (**696 keys × 3**), frontend production build succeeds with **0 warnings in `src/red-cross-qa/`**.
