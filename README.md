...
| **QA & Test Automation** | Red Cross Web QA Agent (20 tabs / 37 endpoints): Playwright + Cypress generators, Skjemabygger + Fundy forms QA, content migration audit, Enonic-specific perf, Designsystemet (Digdir) compliance, role permission matrix, **Azure DevOps work-item bundle**, **Sprint Report generator**, **DPIA / DoD verifier / Resilience / UAT-støtte / Risk Matrix**, **k6 + Loadster dual load-testing**, **Storybook + Postman + GraphQL introspection** (Phase F), **Phase G — NVDA screen-reader script generator + WAVE (WebAIM) audit** as parallel accessibility tools, aligned to Trine Bruu's Teststrategi 30.3 |
...
**i18n**: Full EN / NO / ES parity (40+ top-level sections each, **590 keys per locale** after Phase G). New blocks: `dpia:` (10 keys), `dod:` (15), `resilience:` (13), `uatSupport:` (22), `riskMatrix:` (24), `stakeholders:` (3), Phase C migration provenance + WCAG version (8), Phase D Loadster tool selector (11) + 2 tab labels, Phase F: scenarioStorybook + Tom-tip banners on Playwright/Cypress/Dashboard + GraphQL introspection + Postman panels (27), **Phase G: accessibility tool selector + NVDA scopes + NVDA viewer + WAVE stats + 3 detail tables (29 keys)**.
...
**Validation status**: Backend imports clean (23 suites, 37 routes), end-to-end smoke test passes (`backend/tests/smoke_red_cross_qa.py` — **20 checks**: settings shape, test plan with `test_level`, ADO bundle Sev/Kat, 9+ Fundy checks, sprint report narrative, **DPIA 12 checks + dpia_score, DoD 4-point per work item, Resilience score + breakpoint VU, UAT scripts for Hilde/Trine/Astri, Risk Matrix CSV parser, WCAG 2.1/2.2 version gating, Migrert vs Nyopprettet provenance, Loadster script generator + browser-level run with hydration_p95_ms, Phase F Playwright Storybook scope + Postman Collection v2.1 export + GraphQL introspection, Phase G NVDA script generator (10 steps + 7 WCAG SC + keystrokes Insert+T/Insert+F7/Tab) + WAVE audit (errors/contrast/alerts/features/structure/aria categories + deep link to wave.webaim.org)**), all 3 locales parse with parity (**590 keys × 3**), frontend production build succeeds with **0 warnings in `src/red-cross-qa/`**.
