...
| Andrés the Robot (V0–V1) | 11/11 contract tests (offline, auth+Mongo+LLM mocked; chat, memory CRUD + retrieval) | `python -m pytest backend/tests/test_andres_robot_contracts.py -v` |
...
|-----|-------------------|
| `backend` | `python -m compileall backend` (syntax gate for all backend sources) + `pip install -r backend/requirements.txt` (dependency resolution) + **`pytest` on the offline, mock-first suites** (132 tests) |
