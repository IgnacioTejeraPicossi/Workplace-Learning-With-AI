...
| Andrés the Robot (V0–V5) | 50/50 contract tests (offline, auth+Mongo+LLM mocked; chat, memory, reflection, curiosity, projects + lifecycle (approve-gate, archive-requires-closure, cemetery/compost), evolution + trait cap, creativity + eval, skills safety-gate + sandbox + adversarial gallery, capsule export/preview-diff/import-reversible, development initiative suggest + accept→proposed-project) | `python -m pytest backend/tests/test_andres_robot_contracts.py -v` |
...
|-----|-------------------|
| `backend` | `python -m compileall backend` (syntax gate for all backend sources) + `pip install -r backend/requirements.txt` (dependency resolution) + **`pytest` on the offline, mock-first suites** (171 tests) |
