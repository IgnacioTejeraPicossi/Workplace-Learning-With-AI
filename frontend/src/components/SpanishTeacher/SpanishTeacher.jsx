...
// ═══════════════════════════════════════════════════════════════════════════════
// Tab: Conversación en audio — práctica hablada con manos libres
// ---------------------------------------------------------------------------
// Hablas (Web Speech ASR, es-ES) → el profe responde (mismo endpoint que la
// pestaña Conversación escrita) → la respuesta se lee en voz alta (TTS, con
// soporte de Voicebox / voz clonada vía useVoice). Reusa
// /api/spanish/conversation/message; sin cambios en el backend. Réplica de la
// pestaña Conversation Audio de English Mastery AI y Japanese Sensei AI.
// ═══════════════════════════════════════════════════════════════════════════════
const TabConversationAudio = () => {
  const { t, i18n } = useTranslation();
  const lang = toLang(i18n.language);
  const tts = useVoice();
  const asr = useSpeechCapture({ lang: 'es-ES', interim: true });
  ...
  const start = async () => {
    ...
    setSending(true);
    try {
      const r = await fetch(`${API_BASE}/api/spanish/conversation/message`, {
  ...
  const toggleMic = () => {
    ...
    tts?.stop?.();          // don't record the teacher's own voice
    asr.startListening();
  };
  ...
  return (
    <div style={{ padding: '28px 32px', maxWidth: 920, margin: '0 auto' }}>
      <SectionLabel index={10} label={t('spanishTeacherModule.tabs.conversationAudio')} />
      <h2 style={{ color: C.ink, fontSize: 22, fontWeight: 800, margin: '0 0 4px' }}>{t('spanishTeacherModule.conversationAudio.title')}</h2>
      <p style={{ color: C.inkSoft, fontSize: 13, marginBottom: 18 }}>{t('spanishTeacherModule.conversationAudio.subtitle')}</p>

      {!asr.isSupported && (
        <Card accent={C.gold} style={{ background: C.goldLight || '#fffbeb', borderColor: '#fde68a', marginBottom: 16 }}>
          <p style={{ margin: 0, fontSize: 12.5, color: '#78350f' }}>⚠ {t('spanishTeacherModule.conversationAudio.noAsr')}</p>
        </Card>
      )}
      ...
      {!started ? (
        <div style={{ display: 'grid', gridTemplateColumns: '1.4fr 1fr 1fr', gap: 12, alignItems: 'end', marginBottom: 16 }}>
          ...
          <Button primary onClick={start} disabled={sending}>🎙 {t('spanishTeacherModule.conversationAudio.startBtn')}</Button>
        </div>
      ) : (<>
        ...
        <div style={{ minHeight: 22, marginBottom: 10, fontSize: 13, color: asr.isListening ? C.accent : C.inkSoft, fontStyle: 'italic' }}>
          {asr.isListening
            ? `🎙 ${asr.transcript || t('spanishTeacherModule.conversationAudio.listening')}`
            : (sending ? t('spanishTeacherModule.conversationAudio.thinking') : '')}
        </div>
        ...
        <div style={{ display: 'flex', gap: 10, alignItems: 'center', flexWrap: 'wrap' }}>
          <button
            ...
            style={{
              ...
              background: asr.isListening ? '#dc2626' : C.accent, color: '#fff',
              boxShadow: asr.isListening ? '0 0 0 4px rgba(220,38,38,0.2)' : '0 2px 8px rgba(170,21,27,0.2)',
            }}
          >
            {asr.isListening
              ? `⏹ ${t('spanishTeacherModule.conversationAudio.stopSpeaking')}`
              : `🎙 ${t('spanishTeacherModule.conversationAudio.speak')}`}
          </button>
          <label style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 12.5, color: C.inkSoft, cursor: 'pointer' }}>
            <input type="checkbox" checked={autoSpeak} onChange={(e) => setAutoSpeak(e.target.checked)} />
            {t('spanishTeacherModule.conversationAudio.autoSpeak')}
          </label>
          <Button onClick={reset} style={{ marginLeft: 'auto' }}>{t('spanishTeacherModule.conversation.newConversation')}</Button>
        </div>
      </>)}
    </div>
  );
...
const SpanishTeacher = () => {
  ...
  const tabs = [
    ...
    { id: 'conversationAudio', label: t('spanishTeacherModule.tabs.conversationAudio'), icon: '🎙' },
