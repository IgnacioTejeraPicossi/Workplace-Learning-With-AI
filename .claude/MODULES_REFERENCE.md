...
| 13 | [Red Cross Web QA Agent](#13-red-cross-web-qa-agent) | Active | **CI-gated (1.42.0)**: `test_red_cross_qa_smoke.py` wraps `smoke_red_cross_qa` (~330 asserts) + `smoke_qa_security` | Medium |
| 14 | [AGI Progress Hub + Homo vs. AI Workshop](#14-agi-progress-hub--homo-vs-ai-workshop) | Active (6 tabs) | **CI-gated (1.42.0)**: `test_homo_vs_ai_smoke.py` wraps feedback-log + prompt-evolution + istqb-rag-translation smokes | Low |
| 15 | Web Lab (1.16.0 · V0 structure only) | Active (placeholder) | None yet (V1+) | Low (V0 frontend-only) |
| 16 | Self-Simulating Reality Agent (1.38.0 · V0+V1+V2+V3) | Active (curated content + **conversational chat** + **real vector store / Source Map** + **Compare Theories** + **Red Team** + **Learning Path** + backend claim analyzer + live physics search + interactive playground) | `test_self_sim_reality_chat.py` (6) + `_vectorstore.py` (7) + `_compare.py` (4) + `_redteam.py` (3) + `_learning.py` (4 offline: evidence-first order, goal→stage routing, endpoint, 422) = 24 offline + `/api/claim-analyzer/analyze` smoke via TestClient | Medium — philosophical/scientific companion for Observer Patch Holography; **all 6 planned endpoints shipped (chat, source-map, vectorstore health, compare-theories, red-team, learning-path)**; **15 tabs**: Overview · **🎓 Learning Path** (V3 [1.38.0] — `GET /api/self-sim-reality/learning-path?goal=`, `backend/services/self_sim_reality_learning.py`; deterministic evidence-first 9-stage curriculum (established→mainstream→speculative OPH→philosophy→practice), each stage → tab + level + KB concept ids; optional goal routed to entry stage via vector store (tfidf), earlier stages shown as prerequisites; stage text localized in i18n; `LearningPath.jsx`) · Core Concepts · OPH Mechanics · Theory Tour (9 theories incl. **Celestial Holography** + **CTMU** (Langan · badge `philosophy`, honest fringe caveat) + **Featured Voice Sabrina Gonzalez Pasterski**, Perimeter Institute · Simons Collaboration Deputy Director) · **💬 Dialogue** (V1 [1.36.0] — `POST /api/self-sim-reality/chat`, `backend/services/self_sim_reality_chat.py` + `routers/self_sim_reality.py`; structured JSON short_answer + level-tagged sections + red-team objections + safer_reformulation + next-question; off-palette levels clamped; trilingual mock; `frontend/src/self-sim-reality/Dialogue.jsx`) · **🗂️ Source Map** (V2 [1.37.0] — `POST /api/self-sim-reality/source-map`, `backend/services/self_sim_reality_vectorstore.py`; **real vector store** over the 23-chunk KB (12 base + 11 hand-curated OPH-commentary chunks [1.38.2]; OPH `book/` is CC BY-NC-SA so NOT bulk-ingested — paraphrased commentary w/ chapter attribution, OPH chunks stay speculative) with two backends: OpenAI `text-embedding-3-small` embeddings (cached, semantic) + pure-Python TF-IDF cosine (deterministic/offline fallback); topic → ranked sources with EpistemicBadge + relevance bar; chat retrieval also routed through it (tfidf); `SourceMap.jsx`) · **⚖️ Compare** (V2 [1.37.1] — `POST /api/self-sim-reality/compare-theories`, `backend/services/self_sim_reality_compare.py`; each side grounded via vector store (tfidf → title + level), LLM → agreements/differences/relation/honest_note; levels + relation clamped; never picks a winner; trilingual mock; `CompareTheories.jsx`) · WiPhy Search (live queries against `wiphy.org/api/search`, Pasterski's public physics-claims corpus) · Claim Analyzer (backend LLM decomposes strong claims into scientific core / overreach / reformulation with 5 overreach types; cross-tab bridge to WiPhy) · **⚔️ Red Team** (V2 [1.37.2] — `POST /api/self-sim-reality/red-team`, `backend/services/self_sim_reality_redteam.py`; good-faith challenge: steelman-first + typed objections (empirical/logical/conceptual/methodological/parsimony) w/ strength + what-would-change-my-mind + surviving-core + verdict; grounded via vector store; never "disproven"; clamped; trilingual mock; `RedTeam.jsx`) · AI as Observer · Substrate Question · Playground (SVG Theory Map with 8 nodes + 9 typed edges, HTML5 Canvas Observer Patch simulator with brownian motion + overlap consensus metric) · Roadmap. EpistemicBadge enforces 5-level discipline (`established/mainstream/speculative/philosophy/metaphor/unsupported`). See `docs/self-sim-reality-agent-plan.md` |
