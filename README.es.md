...
### Andrés the Robot — compañero de IA en desarrollo (agosto de 2026)

**Andrés the Robot** (barra lateral → **Future Item Agents → Andrés the Robot**) es un
compañero de IA en desarrollo que construye una **biografía digital verificable y
reversible** (memoria, identidad versionada, reflexión, creatividad, habilidades en
sandbox) sobre un modelo de lenguaje — explícitamente **no** una afirmación de conciencia.
Habla por texto, voz y un avatar 3D opcional, y puede mirar **una imagen por mensaje**
(percepción limitada con consentimiento por turno, condicionada por el nivel de
investigación "Documentos"). Todo lo que recuerda o cambia es **propuesto → revisado →
aprobado por ti**.
**Guía de usuario:** [docs/andres-robot-help.md](docs/andres-robot-help.md).
...
5. 💬 **Dialogue / Diálogo** *(NUEVO · V1 [1.36.0])* — el agente conversacional. Pregunta sobre observadores, mentes, consciencia, simulación, holografía u OPH; **cada respuesta vuelve estructurada y etiquetada por nivel de evidencia** — nunca prosa suelta que pueda hacer pasar la especulación por hecho. `POST /api/self-sim-reality/chat` ancla cada turno en una **base de conocimiento curada OPH + ciencia** (12 chunks, cada uno con su nivel epistémico + citas) recuperada por solapamiento de palabras clave («RAG-lite» — un vector store es el enganche de mejora documentado). JSON estructurado: `short_answer` + `sections` etiquetadas por nivel (scientific_grounding / speculative_extension / oph_interpretation) + `objections` (red-team) + `safer_reformulation` (cuando la pregunta se sobre-extiende) + `suggested_next_question`. Los niveles fuera de paleta se recortan; el mock trilingüe mantiene viva la pestaña offline. El prompt de sistema aplica la regla del módulo (*nunca «esto es verdad», siempre «esto pertenece al nivel X»*; OPH siempre `speculative`) y señala la confusión observador-como-dispositivo vs observador-como-mente.
6. 🔍 **WiPhy Search** — consultas en vivo contra `wiphy.org/api/search`, el servidor MCP público de Pasterski para recuperación de afirmaciones de física (~10 155 papers · 361 273 afirmaciones · 17 953 conceptos). Bloque de estadísticas del corpus al montar + parser JSON defensivo (acepta múltiples formas de nombre de campo y envoltorio) + textos de error diferenciados `cors` / `http` / `network` con escape "Open on wiphy.org". Los IDs de paper enlazan a arXiv.
7. 🔬 **Claim Analyzer** *(V2)* — pega una afirmación fuerte (p. ej. *"la consciencia colapsa la función de onda"*) y el LLM del backend devuelve 5 paneles: badge de **veredicto epistémico** (`mostly_solid / mixed / mostly_overreach / unsupported`), **núcleo científico** (cada parte con su nivel de evidencia), **exceso** (cada parte etiquetada con uno de 5 tipos: `unsupported / category_error / conflation / overgeneralization / philosophical_leap`), **reformulación** en el registro honesto de la misma idea, y **términos clave** como chips clicables. Al pulsar un término se puentea a la pestaña WiPhy Search con la búsqueda auto-ejecutada vía un estado de shell `{query, nonce}`.
8. 🎨 **Playground** *(V3)* — dos herramientas educativas emparejadas apiladas verticalmente: **Theory Map** (SVG puro, 8 nodos con OPH centrado y 7 satélites posicionados a mano por afinidad estructural; 9 aristas tipadas con colores distintos: `provides_form`, `structural_parallel`, `competes_with`, `candidate_measure`, `de_mystifies`, `different_framing`, `supports_side`, `extends_to_flat`; clic en un nodo → panel de info que lee de las mismas claves i18n que Theory Tour) y **Observer Patch Simulator** (HTML5 Canvas 720×380 con limpieza de `requestAnimationFrame` + `cancelAnimationFrame`; N patches con movimiento browniano + rebotes en paredes + convergencia de estado por solapamiento por pares; métrica de consenso en vivo `1 - std(states)` que crece hacia el 100% sin ningún coordinador global — el remate pedagógico).
9. 🧠 **AI as Observer** — 5 experimentos mentales sobre si las IAs pueden ser observer patches
10. 🌌 **Substrate Question** — 7 secciones filosóficas que extienden la ontología de OPH (problema difícil, sustrato vs experiencia, convergencia cosmológica, hipótesis de comprensión recursiva, pregunta platónica, frontera lingüística, tres posiciones honestas)
11. 🗺️ **Roadmap & Sources** — fases V0→V3 + 14 enlaces de referencia (OPH repo/learn/book, Mueller X, Bostrom, Rovelli, Susskind, Friston, IIT, GNW, **Pasterski Perimeter, Simons Collaboration, arXiv 2111.11392, physicsgirl.com**) + bloque de integraciones candidatas para la integración completa de la herramienta MCP `wiphy.org/mcp` (pendiente de trabajo de backend)

**Backend**:
- `backend/services/self_sim_reality_chat.py` + `backend/routers/self_sim_reality.py` *(V1 [1.36.0])* — `POST /api/self-sim-reality/chat` (+ `GET /concepts`, `GET /health`). Base de conocimiento curada de 12 chunks recuperada por solapamiento de palabras clave (RAG-lite), JSON estructurado etiquetado epistémicamente con sanitizado/recorte, mock trilingüe. Tests `backend/tests/test_self_sim_reality_chat.py` (6 offline).
...
- **Pestaña 1 — AGI Progress Tracker**: marco inspirado en CHC de Hendrycks et al. (2025). El dataset abarca ahora **14 modelos, actualizado hasta agosto de 2026** (con datos de la web donde supera el corte de conocimiento del asistente): GPT-4 (2023) 27% → GPT-5 (2025) 58% → **Claude Opus 4.6/4.7/4.8** (2025–26) 61/67/71% → **Gemini 3.1 Pro** 61% → los chinos de peso abierto **Kimi K2.6/K3** (Moonshot) 59/66% → **GPT-5.6 «Sol»** 75% → **Claude Opus 5** 76% → **Gemini 3.6 Flash** 66% → **Grok 4.5/4.6** (xAI/SpaceXAI) 68/73% → **Claude Fable 5** 80% (requiere créditos de uso). Cada modelo lleva notas de benchmark reales (GPQA Diamond, SWE-bench Verified/Pro, ARC-AGI-2, Artificial Analysis Intelligence Index). Long-Term Memory Storage permanece en 0 en todos los LLMs actuales — el cuello de botella arquitectónico del paper. Matiz honesto en las notas: los modelos text-first de peso abierto (Kimi) pueden liderar benchmarks de código pero puntúan más bajo en *esta* métrica de amplitud porque visión y audio pesan igual que el resto de dominios.
